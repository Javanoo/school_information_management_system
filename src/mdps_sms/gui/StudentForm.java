...
public class StudentForm extends Form {
	...
	StudentForm(TreeSet<SchoolClass> classrooms){
		super();
		...
		this.classrooms.setItems(FXCollections.observableList(new LinkedList<>(classrooms)));
	...
	public boolean createNew(TreeSet itemList) {
		if(verifyEntries(new Control[] {codeNumber,name, gender, parent},
						new String[] {"Enter student code","Enter name", "Choose gender", "Enter parent"})) {
			...
			if(selectedClass != null && parentArray != null) {
				itemList.add(new Student(name.getText(), codeNumber.getText(),gender.getValue().getText(), selectedClass, parentArray, description));
				return true;
			}
	...
	public Parent[] createParents(TextField firstParent, TextField firstParentPhone, TextField firstParentPhone2,
		...
		String sParentP1 = secondParent.getText();
		String sParentP2 = secondParent.getText();
		String sParentE1 = secondParent.getText();
		String sParentE2 = secondParent.getText();
		...
		if(!fParent.isBlank()) {
			...
			if(!fParentP1.isBlank() && verifyPhone(fParentP1)) {
				...
				if(!sParent.isBlank()) {
					if(!fParentP2.isBlank() && verifyPhone(fParentP1)) {
	...
	public <E extends Person> void edit(Student person, TreeSet itemList) {
		save.setText("Save");
